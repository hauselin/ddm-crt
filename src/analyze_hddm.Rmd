...
```{r}
fit_ddm <- fit_ddm[country == 1 & result == "success", .(id, Country = country, drift, B, x0, nondectime, rt, acc, rt0, rt1, acc_pred, rt_pred, rt0_pred, rt1_pred, result)]
fit_ddm

params <- data.table(fit_hddm$allIndPars)
params$Country <- 1
params
...
fit_ddm2 <- fit_ddm[result == "success", ]
cor_method <- "spearman"
p11 <- plot(cor_test(fit_ddm2, "acc_pred", "acc", cor_method)) + geom_abline(lty = "dashed", col = 'red')
p12 <- plot(cor_test(fit_ddm2, "rt_pred", "rt", cor_method)) + geom_abline(lty = "dashed", col = 'red')
p13 <- plot(cor_test(fit_ddm2, "rt0_pred", "rt0", cor_method)) + geom_abline(lty = "dashed", col = 'red')
p14 <- plot(cor_test(fit_ddm2, "rt1_pred", "rt1", cor_method)) + geom_abline(lty = "dashed", col = 'red')
p15 <- (p11 + p12) / (p13 + p14)
p15
